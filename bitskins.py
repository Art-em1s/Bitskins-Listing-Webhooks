    ...
    try:
        ...
        if item_average is None:
            ...
            if price < float(item_average[0]):
                post_message(data, item_float, item_average, 0x00ff00, item_stickers, seed, insp)
                if last_post+60 < time.time():
                    post_message_free(data, item_float, item_average, 0x00ff00, item_stickers, seed, insp)
                ...
                if price < item_average[3][1]:
                    ...
                    if delta >= 5:
                        post_message(data, item_float, item_average, 0xeeffff, item_stickers, seed, insp)
                        if last_post+60 < time.time():
                            post_message_free(data, item_float, item_average, 0xeeffff, item_stickers, seed, insp)
                        
            if item_float is not None:
                if float(item_float) <= 0.009:
                    post_message(data, item_float, item_average, 0x0000ff, item_stickers, seed, insp)
                    if last_post+60 < time.time():
                        post_message_free(data, item_float, item_average, 0x0000ff, item_stickers, seed, insp)
            ...
            if item_stickers is not None:
                if any(sticker in str(item_stickers) for sticker in good_stickers):
                    post_message(data, item_float, item_average, 0xff0000, item_stickers, seed, insp)
                    if last_post+60 < time.time():
                        post_message_free(data, item_float, item_average, 0xff0000, item_stickers, seed, insp)
                        
            if seed is not None:
                if int(seed) in ch_trigger_patterns and "case hardened" in name.lower():
                    post_message(data, item_float, item_average, 0x85ffff, item_stickers, seed, insp)
                    if last_post+60 < time.time():
                        post_message_free(data, item_float, item_average, 0x85ffff, item_stickers, seed, insp)
                else:
                    if "case hardened" in name.lower():
                        post_message(data, item_float, item_average, 0x778899, item_stickers, seed, insp)
...
def post_message_free(d,fv,avg,c,stickers,seed=None, insp=None):
    try:
        global last_post
        if last_post:
            if int(last_post+60-time.time()) > 1:
                return
        ...
        if app_id == 730:
            ...
            if d['phase'] is not None:
                msg+="Phase: {}\n".format(str(d['phase']).replace("phase", "Phase "))
            if fv is not None:
                if fv == "-1.0000000000":
                    pass
                elif fv != "None":
        ...
        if avg[3][1]:
            delta = float(avg[3][1]) - price
            perc = delta / price * 100
        else:
            delta = float(avg[0]) - price
            perc = delta/price*100
        if price > 0.07 and c == 0x00ff00:    
            return
        if int(perc) <= 5 and c == 0x00ff00:
            return
...
def post_message(d,fv,avg,c,stickers,seed=None, insp=None):
    try:
        app_id_name = next((item for item in app_id_lookup if item['a'] == d['app_id']), None)
